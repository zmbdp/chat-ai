...
chat-ai 集成了 **Prometheus + Grafana + AlertManager** 作为服务监控与告警解决方案，提供：
...
```bash
cd deploy/dev/app
docker compose -p chat-ai -f docker-compose-mid.yml up -d chat-ai-prometheus chat-ai-grafana chat-ai-alertmanager
```
...
登录 Grafana 后，在左侧菜单选择 "Dashboards" → "chat-ai"，可以看到：
...
4. 选择 "Prometheus"
5. 配置 URL：`http://chat-ai-prometheus:9090`
6. 点击 "Save & Test"
...
```bash
# 在 Grafana 容器中测试连接
docker exec -it chat-ai-grafana curl http://chat-ai-prometheus:9090/-/healthy
```
...
3. 查看 AlertManager 日志：`docker logs chat-ai-alertmanager`
...
2. 查看 AlertManager 日志：
   ```bash
   docker logs chat-ai-alertmanager
   ```
