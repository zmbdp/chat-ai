...
```bash
cd deploy/dev/app
docker compose -p chat-ai -f docker-compose-mid.yml up -d chat-ai-skywalking-oap chat-ai-skywalking-ui
```
...
ENV SW_AGENT_COLLECTOR_BACKEND_SERVICES="chat-ai-skywalking-oap:11800"
...
chat-ai 已集成 SkyWalking Logback 插件，日志会自动关联 TraceId。
...
```bash
# 查看 OAP 日志
docker logs chat-ai-skywalking-oap
